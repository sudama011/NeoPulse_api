...
async def run_morning_drill():
    ...
    try:
        ...
        csv_url = neo_client.get_scrip_master(segment=EXCHANGE_NSE)
    ...
    for row in reader:
        try:
            ...
            if segment != EXCHANGE_NSE:  # e.g. 'nse_cm'
                continue
            ...
            record = {
                ...
                "segment": segment,
                "freeze_qty": (
                    int(row.get("lFreezeQty", 0)) if row.get("lFreezeQty") else None
                ),
    ...
    if not batch_data:
        logger.warning(
            f"⚠️ No records parsed! Skipped: {count_skipped}. Headers found: {cleaned_headers}"
        )
        return
