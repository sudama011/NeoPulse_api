...
async def test_sync_position(strategy):
    mock_resp = {"data": [{"instrumentToken": "123", "netQty": "50", "avgPrice": "1500.0"}]}
...
async def test_buy_entry_checks_risk(strategy):
    ...
    with patch("app.strategy.base.risk_manager") as mock_risk, patch("app.strategy.base.execution_engine") as mock_exec:
        ...
        mock_exec.execute_order = AsyncMock(
            return_value=OrderResponse(order_id="1", status=OrderStatus.COMPLETE, filled_qty=10)
        )
...
async def test_sell_exit_skips_risk_check(strategy):
    ...
    with patch("app.strategy.base.risk_manager") as mock_risk, patch("app.strategy.base.execution_engine") as mock_exec:
        ...
        mock_exec.execute_order = AsyncMock(
            return_value=OrderResponse(order_id="2", status=OrderStatus.COMPLETE, filled_qty=50)
        )
