...
from app.core.circuit_breaker import limits_circuit_breaker, positions_circuit_breaker
...
class StrategyManager:
    ...
    async def configure(self, strategy_name: str, symbols: List[str], params: Dict) -> None:
        """
        ...
        self.capital_manager = CapitalManager(total_capital=self.available_capital, risk_per_trade_pct=risk_per_trade)
        logger.info(
            f"✅ CapitalManager initialized: Capital=₹{self.available_capital:,.2f}, Risk={risk_per_trade * 100:.1f}%"
        )
    ...
    async def _add_strategy_unsafe(self, strategy_class: Type, symbol: str, token: str) -> None:
        """
        ...
        if token not in self.strategies:
            try:
                ...
                strategy = strategy_class(symbol=symbol, token=token, capital_manager=self.capital_manager)
            ...
            async with self._strategies_lock:
                for token, strategy in self.strategies.items():
                    if strategy.position != 0:
                        ...
                        tasks.append(order_executor.place_order(strategy.symbol, token, side, qty, price=0.0))
    ...
    async def reconcile(self) -> None:
        ...
        if not settings.PAPER_TRADING:
            try:
                ...
                limits = await limits_circuit_breaker.call(run_blocking, neo_client.get_limits)
        ...
        if not settings.PAPER_TRADING:
            try:
                # ✅ run_blocking wrapper + circuit breaker for sync Neo API
                positions_resp = await positions_circuit_breaker.call(run_blocking, neo_client.get_positions)
    ...
    async def _process_ticks(self) -> None:
        ...
        while self.is_running:
            try:
                ...
                tick_payload = await asyncio.wait_for(event_bus.tick_queue.get(), timeout=2.0)
    ...
    async def _heartbeat_loop(self) -> None:
        ...
        while self.is_running:
            try:
                ...
                async with self._strategies_lock:
                    tasks = [strategy.on_time_update(now) for strategy in self.strategies.values()]
                    if tasks:
