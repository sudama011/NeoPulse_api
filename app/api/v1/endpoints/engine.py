...
async def start_bot(data: StartRequest, background_tasks: BackgroundTasks, session=Depends(get_session)):
    ...
    try:
        ...
        return {"status": "error", "message": f"Database error while fetching instrument tokens: {str(e)}"}

    # Configure strategy engine with validated symbols and parameters
    try:
        await strategy_engine.configure(strategy_name=data.strategy, symbols=data.symbols, params=data.strategy_params)
        logger.info(f"✅ Strategy engine configured with {len(data.symbols)} symbols")
    except ValueError as e:
        logger.error(f"❌ Strategy configuration error: {e}")
        return {"status": "error", "message": f"Strategy configuration failed: {str(e)}"}
    except Exception as e:
        logger.error(f"❌ Unexpected error during strategy configuration: {e}")
        return {"status": "error", "message": f"Failed to configure strategy: {str(e)}"}
    ...
    return {"status": "success", "message": "Bot configured and started.", "config_id": config_entry.id}
