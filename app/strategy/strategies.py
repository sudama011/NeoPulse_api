...
class RuleBasedStrategy(BaseStrategy):
    ...
    """
    def __init__(self, symbol, token, capital_manager, config: dict):
        super().__init__("RULE_ENGINE", symbol, token, capital_manager)
        self.config = config

    async def logic(self, candle: dict):
        if len(self.candles) < 50: return
        ...
        if self.position == 0:
            ...
            for rule in self.config.get('buy_conditions', []):
                ...
                if rule['op'] == '<' and not (val < rule['val']): buy_signal = False
                if rule['op'] == '>' and not (val > rule['val']): buy_signal = False
            
            if buy_signal: await self.execute_order("BUY", close, "RULE_BUY")
        ...
        # Evaluate SELL (Exit)
            ...
            for rule in self.config.get('sell_conditions', []):
                val = rsi if rule['ind'] == 'rsi' else ema
                if rule['op'] == '>' and (val > rule['val']): sell_signal = True
            
            if sell_signal: await self.execute_order("SELL", close, "RULE_SELL")
...
class MomentumStrategy(BaseStrategy):
    async def logic(self, candle: dict):
        ...
        # Exit (Fixed Targets + Trailing handled in Base)
            ...
            if self.position < 0: pnl_pct *= -1
            
            if pnl_pct > 0.01: await self.execute_order("SELL" if self.position > 0 else "BUY", close, "TP")
            elif pnl_pct < -0.005: await self.execute_order("SELL" if self.position > 0 else "BUY", close, "SL")


# --- 3. OPENING RANGE BREAKOUT (ORB) ---
class ORBStrategy(BaseStrategy):
    def __init__(self, symbol, token, capital_manager):
        super().__init__("ORB", symbol, token, capital_manager)
        self.range_high = 0.0
        self.range_low = 0.0
