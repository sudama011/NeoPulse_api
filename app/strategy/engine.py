...
from app.strategy.strategies import (MomentumStrategy, ORBStrategy, MeanReversionStrategy, RuleBasedStrategy)
...
class StrategyEngine:
    ...
    def get_instance(cls):
        if not cls._instance: cls._instance = StrategyEngine()
    ...
    async def start(self, config: dict):
        ...
        self.capital_manager = CapitalManager(config.get('capital', 100000))
        ...
        for symbol, token in token_map.items():
            if strat_name == 'GENERIC':
                s = GenericStrategy(symbol, token, self.capital_manager, config.get('rules', {}))
            elif strat_name == 'MOMENTUM':
                s = MomentumStrategy(symbol, token, self.capital_manager)
            elif strat_name == 'ORB':
                s = ORBStrategy(symbol, token, self.capital_manager)
            elif strat_name == 'MEAN_REVERSION':
                s = MeanReversionStrategy(symbol, token, self.capital_manager)
            elif strat_name == 'RULE_ENGINE':
                s = RuleBasedStrategy(symbol, token, self.capital_manager, config.get('rules', {}))
            else:
    ...
    async def _square_off_all(self):
        ...
        for s in self.strategies.values():
            if s.position != 0:
                ...
                await order_executor.place_order(s.symbol, s.token, side, abs(s.position))
...
# Global Accessor
strategy_engine = StrategyEngine.get_instance()